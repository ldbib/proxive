...
module.exports = function(config) {
  ...
  return function(req, res) {
    ...
    if(path === '/' || path === '') {
      ...
      config.users.validateSignin(req, res, function(userData) {
        config.organizations.getLimitedFromUser(userData.id, function(err, organization) {
          ...
          if (query.redirect){
            var oldUrl = query.redirect;
            var protocol = oldUrl.split('://');
            var domain = '';
            var path = '';
            if (protocol[1].split('/')){
              domain = protocol[1].split('/');
              path = protocol[1].replace(domain[0], '');
            }
            else {
              domain = protocol[1];
            }
            var rewritten = 'http://';
            rewritten+= (protocol[0] === 'https' ? 'h-t-t-p-s.' : 'h-t-t-p.');
            rewritten+= domain[0];
            rewritten+= '.proxive.se';
            rewritten+= path;
            for (var key in query) {
              if (key !== 'redirect'){
                rewritten+= '&' + key + '=' + query[key];
              }
            }
            res.writeHead(302, {
              location: rewritten
      ...
      config.users.signout(req, res, function(err) {
        if(err) {
          console.error(err);
        }
    ...
    else if(path === '/loginEmail') {
      if(query.redirect) {
        var redirect = uri.search.split('redirect=');
        redirect = redirect[1];
        res.end(config.pugGen.run['login-email.pug']({redirectUrl: redirect}));
      }
      else {
        res.end(config.pugGen.run['login-email.pug']({redirectUrl: null}));
      }
    }
    else if(path === '/signup') {
      config.organizations.getAllFiltered(['organization_id', 'organization_name', 'organization_user_agreement'], function(err, organizations) {
        if(err) {
          console.error(err);
          res.writeHead(500, {
            'content-type': 'text/html; charset=utf-8'
        ...
        return res.end(config.pugGen.run['signup.pug']({
          organizations: organizations,
          orgFreetext: config.orgFreetext ? config.orgFreetext.join(',') : ''
        }));
      });
    }
    else if(path === '/remindMe') {
    ...
    else if(path === '/resetPassword' || path === '/resetPassword/') {
      debug('/resetPassword');
      if(!query.u) {
        res.writeHead(302, {
      ...
      let info = config.emailer.validateHmacEmail(query.u);
      debug(info);
      if(info === false) {
        res.writeHead(302, {
          location: 'http://'+req.headers.host+'/remindMe'
      ...
      config.emailer.checkRecoveryKey(info.user, info.data, function(err) {
        ...
        return res.end(config.pugGen.run['reset-password.pug']({key: query.u}));
      });
    }
      ...
      config.users.validateAdmin(req, res, function(userData) {
        config.users.getUsersFromOrg(userData.organizationId, function(err, users) {
          ...
          config.organizations.getAllFiltered(['organization_id', 'organization_name'], function(err, organizations) {
            ...
            res.end(config.pugGen.run['admin-users.pug']({users: users, currentUser: userData, organizations: organizations, org: userData.organizationId}));
          });
        });
      });
    }
      ...
      config.users.validateAdmin(req, res, true, function(userInfo) {
        if (query.latest){
          config.organizations.searchLogs(userInfo.organizationId, false, function(err, logs) {
            if(err) {
              console.error(err);
              res.writeHead(500, {'content-type': 'application/json; charset=utf-8'});
              return res.end('{"error": "Something went wrong!"}');
