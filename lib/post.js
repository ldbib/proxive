...
module.exports = function(config) {
  function handleLoginCallback(err, user, req, res) {
    ...
    req.cookies.set('login.sec', config.users.hmac(user.userId, Math.floor(Date.now()/1000) + 2592000, 'secure'), {
      domain: '.' + req.headers.host,
      httpOnly: true,
      secure: (config.development ? false : true)
    });
    // Set cookie for browsing for 4 hours
    req.cookies.set('login', config.users.hmac(user.userId, Math.floor(Date.now()/1000) + 14400), {
      domain: '.' + req.headers.host,
      httpOnly: true
    });
    res.writeHead(302, {
