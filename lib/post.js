    ...
    req.on('end', function () {
      ...
      if(req.url === '/loginWithUsername') {
        ...
        config.users.validateAdmin(req, res, true, function(userInfo) {
          config.users.getDetails(parseInt(post.id, 10), function(err, user) {
            if(err) {
              console.error(err);
      ...
      } else if(req.url === '/admin/wiuSearchBlock') {
        // We pass true as the third parameter to ensure that the response will be in json if the user isn't authenticated
        config.users.validateAdmin(req, res, true, function(userInfo) {
          if(userInfo.organizationId !== post.id) {
            res.writeHead(403, {'content-type': 'application/json; charset=utf-8'});
          ...
          if(post.url === '' || post.url === '.') {
            config.organizations.getWiuUrls(post.id, function(err, urls) {
              if(err) {
                console.error(err);
                res.writeHead(500, {'content-type': 'application/json; charset=utf-8'});
                return res.end('{"error": "Something went wrong!"}');
            ...
            config.organizations.searchWiuUrls(post.url, post.id, function(err, urls) {
              if(err) {
                console.error(err);
                if(err === 'invalid-input') {
                  res.writeHead(400, {'content-type': 'application/json; charset=utf-8'});
